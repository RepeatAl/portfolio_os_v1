"""
Integration tests for CLI validation commands
"""

import pytest
import argparse
from pathlib import Path
import tempfile
import shutil
import yaml

from cli_validation_commands import ValidationCommands, Colors
from artifact_registry import ArtifactRegistry
from domain_registry import DomainRegistry
from lifecycle_manager import LifecycleManager


@pytest.fixture
def temp_registry_dir():
    """Create temporary directory for test registries"""
    temp_dir = Path(tempfile.mkdtemp())
    yield temp_dir
    shutil.rmtree(temp_dir)


@pytest.fixture
def test_registries(temp_registry_dir):
    """Create test registries with sample data"""
    # Create artifact registry
    artifact_registry_path = temp_registry_dir / "artifact_registry.yaml"
    artifact_data = {
        'artifacts': [
            {
                'artifact_id': 'test_artifact_1',
                'file_path': 'test/file1.py',
                'primary_domain': 'SIGNALS',
                'artifact_type': 'ENGINE',
                'lifecycle_status': 'active',
                'created_date': '2026-01-01',
                'last_modified': '2026-01-01',
                'owner_role': 'SIGNALS domain owner',
                'ssot_relationship': 'implementation',
                'allowed_writers': ['SIGNALS'],
                'allowed_readers': ['ALL']
            },
            {
                'artifact_id': 'test_ssot_1',
                'file_path': 'docs/test_doc.md',
                'primary_domain': 'ARCH',
                'artifact_type': 'SSOT',
                'lifecycle_status': 'canonical',
                'created_date': '2026-01-01',
                'last_modified': '2026-01-01',
                'owner_role': 'ARCH domain owner',
                'ssot_relationship': 'canonical',
                'allowed_writers': ['ARCH'],
                'allowed_readers': ['ALL'],
                'topic': 'test_topic'
            }
        ]
    }
    
    with open(artifact_registry_path, 'w') as f:
        yaml.dump(artifact_data, f)
    
    # Create domain registry
    domain_registry_path = temp_registry_dir / "domain_registry.yaml"
    domain_data = {
        'domains': [
            {
                'domain_id': 'SIGNALS',
                'name': 'Signal Generation',
                'responsibility_scope': 'Generate structured signals',
                'allowed_artifact_types': ['ENGINE', 'DATA_OUT', 'SSOT'],
                'cannot_own': ['REPORT_OUT'],
                'priority': 'core',
                'authority_level': 1
            },
            {
                'domain_id': 'ARCH',
                'name': 'Architecture',
                'responsibility_scope': 'System architecture',
                'allowed_artifact_types': ['SSOT', 'ENGINE', 'CONFIG'],
                'cannot_own': ['REPORT_OUT'],
                'priority': 'surface',
                'authority_level': None
            },
            {
                'domain_id': 'REPORT',
                'name': 'Report Generation',
                'responsibility_scope': 'Generate reports',
                'allowed_artifact_types': ['ENGINE', 'REPORT_OUT', 'SSOT'],
                'cannot_own': ['DATA_OUT'],
                'priority': 'core',
                'authority_level': 4
            }
        ]
    }
    
    with open(domain_registry_path, 'w') as f:
        yaml.dump(domain_data, f)
    
    # Create lifecycle state machine
    lifecycle_path = temp_registry_dir / "lifecycle_state_machine.yaml"
    lifecycle_data = {
        'artifact_types': {
            'ENGINE': {
                'description': 'Engine lifecycle',
                'states': ['planned', 'development', 'active', 'deprecated'],
                'initial_state': 'planned',
                'transitions': [
                    {'from': 'planned', 'to': 'development', 'condition': 'Implementation begins'},
                    {'from': 'development', 'to': 'active', 'condition': 'Production ready'},
                    {'from': 'active', 'to': 'deprecated', 'condition': 'Replaced'}
                ],
                'modifiable_states': ['planned', 'development', 'active'],
                'read_only_states': ['deprecated']
            },
            'SSOT': {
                'description': 'SSOT lifecycle',
                'states': ['draft', 'review', 'canonical', 'deprecated'],
                'initial_state': 'draft',
                'transitions': [
                    {'from': 'draft', 'to': 'review', 'condition': 'Ready for review'},
                    {'from': 'review', 'to': 'canonical', 'condition': 'Approved'},
                    {'from': 'canonical', 'to': 'deprecated', 'condition': 'Superseded'}
                ],
                'modifiable_states': ['draft', 'review', 'canonical'],
                'read_only_states': ['deprecated']
            }
        }
    }
    
    with open(lifecycle_path, 'w') as f:
        yaml.dump(lifecycle_data, f)
    
    # Create registry instances
    artifact_registry = ArtifactRegistry(artifact_registry_path)
    domain_registry = DomainRegistry(domain_registry_path)
    lifecycle_manager = LifecycleManager(lifecycle_path)
    
    return {
        'artifact_registry': artifact_registry,
        'domain_registry': domain_registry,
        'lifecycle_manager': lifecycle_manager,
        'temp_dir': temp_registry_dir
    }


class TestValidateCommand:
    """Tests for validate command"""
    
    def test_validate_all_observers(self, test_registries):
        """Test running all validation observers"""
        # Disable colors for testing
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        args = argparse.Namespace(
            files=None,
            observer=None,
            dry_run=False,
            no_color=True
        )
        
        result = commands.validate(args)
        
        # Should succeed (0) if no warnings, or fail (1) if warnings found
        assert result in [0, 1]
    
    def test_validate_specific_observer(self, test_registries):
        """Test running a specific observer"""
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        args = argparse.Namespace(
            files=None,
            observer='RegistrationValidator',
            dry_run=False,
            no_color=True
        )
        
        result = commands.validate(args)
        
        assert result in [0, 1]
    
    def test_validate_specific_files(self, test_registries):
        """Test validating specific files"""
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        args = argparse.Namespace(
            files=['test/file1.py'],
            observer=None,
            dry_run=False,
            no_color=True
        )
        
        result = commands.validate(args)
        
        assert result in [0, 1]
    
    def test_validate_dry_run_mode(self, test_registries):
        """Test dry-run mode (should always return 0)"""
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        args = argparse.Namespace(
            files=None,
            observer=None,
            dry_run=True,
            no_color=True
        )
        
        result = commands.validate(args)
        
        # Dry-run should always return 0
        assert result == 0
    
    def test_validate_with_invalid_observer(self, test_registries):
        """Test validation with invalid observer name"""
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        # This would normally be caught by argparse, but test the underlying logic
        args = argparse.Namespace(
            files=None,
            observer='InvalidObserver',
            dry_run=False,
            no_color=True
        )
        
        result = commands.validate(args)
        
        # Should fail with error
        assert result == 1
    
    def test_validate_domain_assignment_observer(self, test_registries):
        """Test running domain assignment observer"""
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        args = argparse.Namespace(
            files=None,
            observer='DomainAssignmentValidator',
            dry_run=False,
            no_color=True
        )
        
        result = commands.validate(args)
        
        assert result in [0, 1]
    
    def test_validate_lifecycle_observer(self, test_registries):
        """Test running lifecycle observer"""
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        args = argparse.Namespace(
            files=None,
            observer='LifecycleValidator',
            dry_run=False,
            no_color=True
        )
        
        result = commands.validate(args)
        
        assert result in [0, 1]
    
    def test_validate_boundary_awareness_observer(self, test_registries):
        """Test running boundary awareness observer"""
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        args = argparse.Namespace(
            files=None,
            observer='BoundaryAwarenessValidator',
            dry_run=False,
            no_color=True
        )
        
        result = commands.validate(args)
        
        assert result in [0, 1]
    
    def test_validate_ssot_consistency_observer(self, test_registries):
        """Test running SSOT consistency observer"""
        Colors.disable()
        
        commands = ValidationCommands(
            test_registries['artifact_registry'],
            test_registries['domain_registry'],
            test_registries['lifecycle_manager'],
            test_registries['temp_dir']
        )
        
        args = argparse.Namespace(
            files=None,
            observer='SSOTConsistencyValidator',
            dry_run=False,
            no_color=True
        )
        
        result = commands.validate(args)
        
        assert result in [0, 1]


if __name__ == '__main__':
    pytest.main([__file__, '-v'])
