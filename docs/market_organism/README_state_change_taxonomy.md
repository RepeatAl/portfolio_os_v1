...
This section declares the relationship between the State_Change_Taxonomy and the Signal Architecture. These are DECLARATIONS of compatibility — they state what the taxonomy preserves and respects. They do NOT implement signal logic.

### Signal Reusability Invariant

State_Change nodes in the Organism_Graph produce Intelligence_Objects that downstream nodes consume through declared Dependency_Paths. The following invariant applies:
...
The State_Change_Taxonomy classifies the impulses that propagate through the organism. Signal_Bubble_v0 signals detect the portfolio-level manifestation of those impulses. The relationship is:
...
4. **Organism relationship**: Signal_Bubble_v0 signals are leaf-node observations (sensors) in the Organism_Graph. They detect that a State_Change (classified by this taxonomy) has propagated to the portfolio level. They are evidence of propagation, not causes of it.
...
4. Any signal referenced in worked examples within this taxonomy must have a Signal_Lifecycle_Definition before implementation.
